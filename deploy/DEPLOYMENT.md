...
subsequent config changes (`create_app`, `modify_app`, `deactivate`).
...
- `TOFU_DIR` — path to `deploy/tofu` in the edgenode repo (or set `--tofu-dir` on each command)
...
# Commands as generated by happ-publisher-ui — example for a deploy action:
...
existing seed.** The network seed is immutable: changing it after bootstrap
orphans all nodes on the network.
