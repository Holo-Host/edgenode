...
mkdir -p /data/holochain/etc /data/holochain/var /data/logs /data/flower
...
if ! grep -q "keystore:" /etc/holochain/conductor-config.yaml || \
   ! grep -q "type: lair_server_in_proc" /etc/holochain/conductor-config.yaml; then
